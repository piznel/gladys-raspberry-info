...
class Proc {
  ...
  get deviceUsb() {
    const devices = exec('find /sys/bus/usb/devices/usb*/ -name dev')
    return devices.split('\n').reduce((info, line) => {
      let syspath = line.Substring(0, line.Length-1);
      let devname = exec(`udevadm info -q name -p ${syspath}`);
      if(devname.Substring(0,4) !== 'bus/') {
        let result = exec(`udevadm info -q property --export -p ${syspath}`)
        console.log(result)
      }
    })
  }
...
function formatBytes(a, b) { if (0 == a) return "0"; var c = 1024,
    d = b || 2,
