...
class Proc {
  ...
  get core() {
    ...
    return (this._cache.core = {
      ...
      serial: (cpuInfo.match(/^serial\s*:\s*(.*)$/im) || [])[1].replace(/^0*/, '') || '',
  ...
  get voltageCpu() {
    ...
    return (Math.round(voltage[1] * 100) / 100)
  }

  get cpuUsage() {
    return exec("{ head -n1 /proc/stat;sleep 0.2;head -n1 /proc/stat; } | awk '/^cpu /{u=$2-u;s=$4-s;i=$5-i;w=$6-w}END{print int(0.5+100*(u+s+w)/(u+s+i+w))}'")
  }
  ...
  get memory() {
    ...
    return memInfo.split('\n').reduce((info, line) => {
      ...
      if (matched) {
        ...
        if (matched[1] === 'MemTotal') info.memoryTotal = matched[2];
        if (matched[1] === 'MemAvailable') info.memoryAvailable = matched[2];
      }
      return info;
    }, {});
  }
  ...
  get connexion() {
    let connexions = [];
    const Listconnexions = exec(`ss -nt '( dport = :443 or dport = :80 or dport = :ssh or dport = :8080 or sport = :443 or sport = :80 or sport = :ssh or sport = :443 )'`).split('\n');
    Listconnexions.shift()
    Listconnexions.pop()
    Listconnexions.forEach((connexion) => {
      let temp = {
        local: (connexion.match(/(\d*\.\d*\.\d*\.\d*:\d*)/) || [])[1] || '',
        peer: (connexion.match(/\d*\.\d*\.\d*\.\d*:\d*\s*(\d*\.\d*\.\d*\.\d*:\d*)/) || [])[1] || '',
        ext: false,
        link: ''
      }
      if (temp && temp.local.substring(0, 9) !== '127.0.0.1') {
        temp.ext = !temp.peer.split(':')[0].match(/(^127\.)|(^10\.)|(^172\.1[6-9]\.)|(^172\.2[0-9]\.)|(^172\.3[0-1]\.)|(^192\.168\.)/)
        if (temp.ext) {
          var whoIs = JSON.parse(exec(`curl -H "Accept: application/json" http://whois.arin.net/rest/ip/${temp.peer.split(':')[0]}`))
          temp.link = `https://whois.arin.net/rest/org/${whoIs.net.orgRef['@handle']}`
        }
        connexions.push(temp)
      }
    })
    return connexions;
  }
  ...
  get openPort() {
    let openPorts = exec(`netstat -lnt | awk 'NR>2{print $4}' | grep -E '(0.0.0.0:|:::|127.0.0.1:|::1:)' | sed 's/.*://' | sort -n | uniq`).split('\n');;

    return {
      8080: openPorts.includes('8080'),
      80: openPorts.includes('80'),
      443: openPorts.includes('443'),
      3306: openPorts.includes('3306'),
      1337: openPorts.includes('1337'),
      22: openPorts.includes('22'),
    }
  }

  get lastConnected() {
    let lastConnected = [];
    const last = exec('last').split('\n');
    last.forEach(line => {
      let temp = {
        user: (line.match(/(^\w+)/) || [])[1] || '',
        fromip: (line.match(/^\w+\s*pts\/\d+\s*(\d*.\d*.\d*.\d*)\s*/) || [])[1] || '',
        info: (line.match(/^\w+\s*pts\/\d*\s*\d*.\d*.\d*.\d*\s*(.*)/) || [])[1] || '',
        ext: false,
        link: ''
      }
      if (temp.fromip) {
        temp.ext = !temp.fromip.split(':')[0].match(/(^127\.)|(^10\.)|(^172\.1[6-9]\.)|(^172\.2[0-9]\.)|(^172\.3[0-1]\.)|(^192\.168\.)/)
        if (temp.ext) {
          var whoIs = JSON.parse(exec(`curl -H "Accept: application/json" http://whois.arin.net/rest/ip/${temp.fromip.split(':')[0]}`))
          temp.link = `https://whois.arin.net/rest/org/${whoIs.net.orgRef['@handle']}`
        }
        lastConnected.push(temp)
      }
    });
  ...
  get diskDetail() {
    ...
    const info = df
      ...
      .map(str => {
        str = {
          file: str[0],
          total: formatBytes(str[1] * 1024, 2),
          used: formatBytes(str[2] * 1024, 2),
          free: formatBytes(str[3] * 1024, 2),
          per_cent: str[4],
          mount: str[5]
        }
        return str
